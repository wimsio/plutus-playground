...
function Explorer({ host }: { host: HostApi }) {
  ...
  // ✅ 1B: on file click call editor.openFile
  async function openFile(path: string) {
    try {
      ...
      // fallback if you wired it on window for now
      const fallback = (window as any).__IDE_OPEN_FILE__;
      if (typeof fallback === "function") {
        await fallback(path);
        return;
      }

      host.ui?.notify?.(`editor.openFile not registered (clicked: ${path})`);
    } catch (e: any) {
