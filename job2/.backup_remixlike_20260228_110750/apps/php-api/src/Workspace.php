...
final class Workspace
{
  ...
  /** Flat list in a folder (for compatibility) */
  public static function listFiles(string $project, string $path): array {
    self::ensureProject($project);
    $dir = self::projectDir($project) . "/" . safe_path($path);
    if (!is_dir($dir)) return [];
    $items = array_values(array_filter(scandir($dir) ?: [], fn($x) => $x !== "." && $x !== ".."));
    return array_map(fn($x) => ($path === "" ? $x : rtrim($path, "/") . "/" . $x), $items);
  }

  /** Tree listing like Remix */
